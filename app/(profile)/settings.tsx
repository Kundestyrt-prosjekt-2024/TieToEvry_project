...
const Settings = () => {
  ...
  const performDelete = () => {
    if (user) {
      deleteUser(user); // Ensure `user.id` matches your schema
      handleLogout();
    ...
    <ScrollView className="flex-grow p-5 bg-white">
      ...
      <View className="flex-row justify-between items-center my-2 py-2 border-b border-gray-300">
        ...
        {user.birthdate?.seconds && (
          <Text className="text-base text-gray-800">
            {formatTimestamp(new Date(user.birthdate.seconds * 1000))}
          </Text>
        )}
      </View>
      ...
      <View className="flex-row justify-between items-center my-2 py-2 border-b border-gray-300">
        ...
        {user.created_at?.seconds && (
          <Text className="text-base text-gray-800">
            {formatTimestamp(new Date(user.created_at.seconds * 1000))}
          </Text>
        )}
      </View>
