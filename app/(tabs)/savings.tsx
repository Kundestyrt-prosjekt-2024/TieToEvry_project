...
const Savings = () => {
  ...
  const { data: savingGoals2, isLoading: isSavingGoalsLoading } = useGetSavingGoals(userIDValue)
        ...
        <ScrollView>
          ...
          <View className="flex-col items-center py-5 pb-20">
            {savingGoals && savingGoals.length > 0 ? (
              savingGoals.map((goal) => <SavingGoalCard key={goal.id} goal={goal} onAddMoney={handleAddMoney} />)
            ) : (
